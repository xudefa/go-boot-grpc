# go-boot-grpc 项目开发规范文档

go-boot-grpc 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 gRPC 框架集成模块。本模块将 gRPC 框架无缝集成到 go-boot 的 IoC 容器和自动配置体系中,遵循 go-boot 项目的开发规范。
...
- **集成模块**:go-boot-grpc 是 go-boot 的 RPC 层集成,将 gRPC 作为 RPC Server/Client 实现
- **规范继承**:完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 gRPC Server/Client 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 gRPC 启动器
- 实现 `net.Server` 接口的 gRPC 适配器
- 支持服务注册、拦截器配置的声明式配置
- 内置分布式追踪拦截器支持
...
- **RPC 框架**:集成 gRPC(`google.golang.org/grpc`)
- **职责边界**:仅负责 gRPC Server/Client 集成,不包含业务逻辑
- **示例代码**:统一放在 `examples/` 目录,演示 gRPC 集成用法

### 2.2 go-boot-grpc 核心包结构

| 包 | 说明 | 核心类型/函数 |
|---|------|----------|
| `grpc/` | gRPC 自动配置 | `GrpcAutoConfiguration` |
| `server/` | gRPC Server 集成 | `Server`, `New`, `WithAddress`, `WithTracing` |
| `client/` | gRPC Client 集成 | `Client`, `New`, `WithAddress`, `WithTimeout`, `WithTracing` |
| `interceptors/` | gRPC 拦截器 | 追踪、日志拦截器 |

### 2.3 go-boot 核心包参考

go-boot-grpc 依赖 go-boot 的以下核心包:
...
| `net/` | HTTP 服务器/客户端抽象接口 | `net.Server` |
| `tracing/` | 分布式追踪抽象 + LocalTracer 实现 | `tracing.Tracer`, `tracing.Span` |

### 2.4 接口抽象原则

go-boot-grpc 遵循 go-boot 的接口抽象原则,所有集成层通过核心框架中的接口抽象定义,实现运行时互换:
...
func init() {
    boot.RegisterAutoConfig(
        ...
        condition.OnProperty("grpc.server.enabled", "true"),
    )
}
```
